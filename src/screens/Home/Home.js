...
function Home() {
  ...
  return (
    <div className="App flex">
      {/* Sidebar */}
      <div className={`fixed inset-0 bg-gray-600 bg-opacity-50 z-20 transition-opacity lg:hidden ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsSidebarOpen(false)}></div>
      <div className={`fixed inset-y-0 left-0 transform bg-white transition-transform lg:relative lg:translate-x-0 lg:w-1/5 border-r-2 border-gray-200 z-30 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <Sidebar />
      </div>
      ...
      {/* Menu Toggle Button */}
      <button className="lg:hidden fixed bottom-4 right-4 bg-primary text-white p-3 rounded-full z-40" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
        ☰
      </button>
