...
import { addToWatchlist, setShouldAddToWatchlistModalOpen } from "../watchlistSlice"
...
export const WatchlistSelectionModal = () => {
    ...
    const handleAddToWatchlist = () => {
        if (listName) {
            ...
            const updatedMovies = [...moviesInTheSelectedList, {...selectedMovie, isWatched: false}]
            const updatedWatchlist = {
        ...
        <Modal title={`Add to Watchlist`} handleClose={handleClose}>
            <div className="w-full">
                <div className="flex pb-4">
                    <div className="w-full">
                        {(watchlist && optionsToShow.length > 0) && (<div className="w-full flex pb-4">
                            <label className="text-xl my-auto mr-4">Add to existing list:</label>
                            <select className="w-1/2 p-2 bg-gray-50 border rounded border-primary/20 focus:border-primary/80 focus:outline-none" onChange={(e) => setSelectedList(e.target.value)}>
                                {optionsToShow.map((key) => <option value={key}>{key}</option>)}
                            </select>
                        </div>
                        )}
                        <div className="w-full flex pt-4">
                            ...
                            <input onChange={(e) => {
                                ...
                                className="w-1/2 p-2 border rounded border-primary/20 focus:border-primary/80 focus:outline-none" type="text" placeholder="List Name" />
                        </div>
                        <div className="w-full flex pt-4 pb-4">
                            <label className="text-xl my-auto mr-4">List Description:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
                            <input onChange={(e) => {
                                setListDescription(e.target.value)
                            }}
                                className="w-1/2 p-2 border rounded border-primary/20 focus:border-primary/80 focus:outline-none" type="text" placeholder="List Description" />
                        </div>
                        {shouldShowError && (<p className="text-red-600">List with same name already exists.</p>)}
                    </div>
                </div>
                ...
                <div className="w-full flex justify-end">
                    ...
                    <button className="bg-green-400 py-2 px-8 rounded" onClick={handleAddToWatchlist}>
                        Add to Watchlist
                    </button>
                </div>
            </div>
