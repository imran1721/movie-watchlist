    ...
    return (<>
        {selectedWatchlist ? (
            <div className="text-left py-4">
                <div className="flex text-4xl items-center font-semibold pb-4">{selectedWatchlist}<div>
                        <img className="ml-4 w-8 h-8" src={editIcon} onClick={() => dispatch(setWatchlistToEdit(selectedWatchlist))}/>
                    </div></div>
                ...
                <div>
                    {watchlist[selectedWatchlist].description}
                </div>
            </div>
        ) : 
